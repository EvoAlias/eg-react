...
it('expands a region in the middle of the genome correctly', () => {
    ...
    let expansion = instance.calculateExpansion(region, region.getWidth()); // 1 base per pixel
    expect(expansion.visRegion.getContextCoordinates()).toEqual({
        start: 5,
        end: 20
    });
    expect(expansion.visWidth).toEqual(15);
...
it('does not modify a region that already spans the entire genome', () => {
    ...
    expect(expansion.visWidth).toEqual(30);
    expect(expansion.viewWindow.start).toEqual(0);
